...
export interface FallbackMetrics {
  ...
  /**
   * v1.4: optional per-weight + per-style targeting. When present, the
   * emitted `@font-face` block carries matching `font-weight` and
   * `font-style` declarations so browsers select the right fallback per
   * face. Unset means a family-wide fallback (the v1.2 behaviour).
   */
  fontWeight?: string;
  fontStyle?: string;
}
...
export function formatFallbackCss(metrics: FallbackMetrics): string {
  const variantTag =
    metrics.fontWeight || metrics.fontStyle
      ? ` (${metrics.fontWeight ?? '400'}${metrics.fontStyle && metrics.fontStyle !== 'normal' ? ' ' + metrics.fontStyle : ''})`
      : '';
  const lines = [
...
/**
 * v1.4: per-face fallback emit. Walk the supplied face list and emit one
 * `<Family> Fallback` block per (family, weight, style) tuple, each pointing
 * at the metrics of the actual primary file for that variant. Closes
 * fontaine #53 (per-weight overrides — open 3+ years) without the
 * Nuxt/Vite coupling.
 *
 * Dedupes by (family, weight, style) — variable fonts producing one
 * binary across many weights still emit one block per weight, all with
 * the same metrics but matching font-weight declarations so the browser
 * selects the right fallback per weight.
 */
export async function buildPerFaceFallbacks(
  filesDir: string,
  faces: Array<{ family: string; weight: string; style: string; sources: Array<{ localFile?: string; format: string | null }> }>,
): Promise<{ css: string; count: number; errors: { file: string; reason: string }[] }> {
  const blocks: string[] = [];
  const errors: { file: string; reason: string }[] = [];
  const seen = new Set<string>();

  // Pre-pass: cache metrics per file so variable fonts (multiple faces sharing one binary)
  // only get parsed once.
  const metricsByFile = new Map<string, FallbackMetrics>();
  for (const face of faces) {
    ...
    try {
      let base = metricsByFile.get(abs);
      if (!base) {
        base = await computeFallback(abs);
        metricsByFile.set(abs, base);
      }
      // Use the face's declared family + weight + style for the emit, not
      // the binary's name table — the CSS targets must match the primary
      // @font-face declarations exactly.
      blocks.push(
        formatFallbackCss({
          ...base,
      ...
      errors.push({ file: primary.localFile, reason: (e as Error).message });
    }
  }
 ...
 *
 * v1.2 / v1.3.1 behaviour kept for direct callers. The pull pipeline
 * now prefers `buildPerFaceFallbacks` so each weight gets its own
 * Capsize block (closes fontaine #53).
 */
